...
const Child2ComponentContent: React.FC = () => {
  const { data, loading, error, errorDetails, retry, canRetry } = useDogApi();
  const [imageError, setImageError] = useState(false);
  const logger = createLogger('child2');

  useEffect(() => {
    logger.info('Child2Component initialized');
  }, [logger]);
...
export const Child2Component: React.FC = () => {
  return (
    <ErrorBoundary
      fallback={
        <div style={{ padding: '20px', textAlign: 'center' }}>
